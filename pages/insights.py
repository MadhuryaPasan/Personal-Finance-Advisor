...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📊 Budget Planner", "🎯 Saving Goals", "💳 Transactions"])
    ...
    with tab1:
        ...
        if budget_df.empty:
            ...
            with col1:
                st.plotly_chart(plot_budget_overview(
                    budget_df), use_container_width=True)
            with col2:
                st.plotly_chart(plot_budget_progress(
                    budget_df), use_container_width=True)
            ...
            if st.checkbox("Show detailed budget table"):
                ...
                if selected_idx is not None:
                    ...
                    st.subheader(
                        f"Budget Details: {selected_budget['Category']}")
    ...
    with tab2:
        ...
        if goals_df.empty:
            ...
            # Visualization
            st.plotly_chart(plot_saving_goals(goals_df),
            ...
            if st.checkbox("Show detailed goals table"):
                ...
                if selected_goal_idx is not None:
                    ...
                    monthly_progress = selected_goal['Net Savings'] / \
                        selected_goal['Monthly Needed'] * \
                        100 if selected_goal['Monthly Needed'] > 0 else 0

                    if monthly_progress >= 100:
                        st.success(
                            f"✅ Exceeding target! You're {monthly_progress:.0f}% of monthly savings needed.")
                    elif monthly_progress >= 75:
                        st.warning(
                            f"⚠️ On track! You're {monthly_progress:.0f}% of monthly savings needed.")
                    else:
                        st.error(
                            f"❌ Below target. You're only {monthly_progress:.0f}% of monthly savings needed.")
    ...
    with tab3:
        ...
        if transactions_df.empty:
            ...
            # Summary metrics
            expenses = transactions_df[transactions_df['Type']
                                       == 'Expense']['Amount'].sum()
            income = transactions_df[transactions_df['Type']
                                     == 'Income']['Amount'].sum()
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.plotly_chart(plot_transactions(
                    transactions_df), use_container_width=True)
            with col2:
                st.plotly_chart(plot_transaction_timeline(
                    transactions_df), use_container_width=True)
            ...
            with col3:
                date_range = st.date_input(
                    "Date Range:",
                    value=(transactions_df['Date'].min().date(
            ...
            if len(filtered_df) > 0:
                selected_trans_idx = st.selectbox(
                    ...
                    format_func=lambda x: f"{filtered_df.iloc[x]['Date'].strftime('%Y-%m-%d')} - {filtered_df.iloc[x]['Category']} (Rs{filtered_df.iloc[x]['Amount']:,.0f})"
                )
                ...
                if selected_trans_idx is not None:
                    ...
                    with col1:
                        st.metric(
                            "Date", selected_transaction['Date'].strftime('%Y-%m-%d'))
