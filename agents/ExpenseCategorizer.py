...
class ExpenseCategorizerAgent:
    def __init__(self, type_model_path: str = "../models/expense_income_type", 
        ...
        try:
            self.nlp_type = spacy.load(type_model_path)  # Model for Expense/Income type
            self.nlp_cat = spacy.load(cat_model_path)    # Model for category
            logging.info("Loaded type model from %s and category model from %s", 
                         type_model_path, cat_model_path)
    ...
    def extract_money(self, text: str) -> str:
        ...
        if m:
            logging.info("Extracted amount '%s' using regex from input '%s'", m.group(0), text)
            return m.group(0)
        ...
        try:
            ...
            for ent in doc.ents:
                if ent.label_.upper() == "MONEY":
                    logging.info("Extracted amount '%s' using NER from input '%s'", ent.text, text)
                    return ent.text
        except Exception:
            pass
